...
namespace SAP_Sync_Api.Controllers
{
    public class PriceListController : ApiController
    {
        ...
        private Recordset oRecSet2 = null;
        public string response { get; set; }

        // GET: PriceList
        [HttpPost]
        [Route("newitem/price")]
        public IHttpActionResult InsertNewPrice(PriceListMaster PriceListData)
        {
            ...
            else
            {
                ...
                try
                {
                    var resp = NewItemPrice(PriceListData);
                    //  return resp;
                    return Ok(resp);
        ...
        public string NewPrice(OPLM PriceListData, string division_code)
        {
            ...
            try
            {
                string DoQuery2 = "select Distinct a.DocEntry from  [@ins_oplm] a  where u_itemcode='" + PriceListData.U_ItemCode + "'";

                oRecSet2 = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                oRecSet2.DoQuery(DoQuery2);
                int DocEntry = oRecSet2.Fields.Item("DocEntry").Value;
                if (DocEntry == 0)
                {
                    ...
                    response = response + PriceListData.U_ItemCode + " - Created, ";
                    return response;
                }
                else
                {
                    response = response + PriceListData.U_ItemCode + " Already Exists in PLM, ";
                    return response;
                }
        ...
        private string UpdatePrice(PriceData priceData, string division_code)
        {
            ...
            if ((division_code.ToUpper() != "ACC"))
            {
                try
                {
                    ...
                    if (priceData.SubBrand.ToString() == "")
                    {
                        //// to get the linenumber
                        DoQuery1 = "select C.LineId,c.U_MRP,c.U_SelPrice,U_FranMRP,U_Franprice ,U_DirMRP,U_Dirprice from  [@ins_oplm] a inner join [@ins_plm1] b on a.docentry=b.docentry " +
                           "inner join [@ins_plm2] c on a.docentry=c.docentry and b.lineid=c.u_rowid   " +
                           "where  U_State='" + priceData.SalePriceCode + "'AND U_BRAND='" + priceData.Brand + "' AND u_itemcode='" + priceData.ItemCode + "'";
                    ...
                    oChildren.Item(LineNum - 1).SetProperty("U_FranMRP", priceData.FranMRP.ToString());
                    oChildren.Item(LineNum - 1).SetProperty("U_Franprice", priceData.FranPrice.ToString());
                    ...
                    oChildren.Item(LineNum - 1).SetProperty("U_DirMRP", priceData.DirMRP.ToString());
                    oChildren.Item(LineNum - 1).SetProperty("U_Dirprice", priceData.DirPrice.ToString());
            ...
            else if (division_code.ToUpper() == "ACC")
            {
                try
                {
